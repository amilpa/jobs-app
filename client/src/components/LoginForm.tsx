    ...
    <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 shadow-lg bg-teritiary py-5 px-3 rounded-lg'>
      <form>
        ...
        <div className='flex gap-2 flex-col md:w-[400px] w-[250px] my-2'>
          ...
          <input type="text" className='border-[1px] border-black rounded-s pl-2 py-1' value={email} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEmail(e.target.value)} />
        </div>
        {emerror ? <Error text='Please enter valid email' /> : ''}
        <div className='flex gap-2 flex-col md:w-[400px] w-[250px] my-2'>
          <label className='text-lg'>Password</label>
          <input type="password" className='border-[1px] border-black rounded-s pl-2 py-1' value={password} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPassword(e.target.value)} />
        </div>
